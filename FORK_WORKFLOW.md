...
`feat/hmk-native-memory-retirement` is also retired as of the 2026-09-21 sync:
upstream now evaluates native `memory` tool availability directly from the
registry without caching transient absence. The historical tip remains under a
verified backup ref; do not compose it into `main`.
