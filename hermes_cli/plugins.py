...
class PluginCommandContext:
    ...
    authorized: bool
    # Optional transport metadata, never credentials or a human-approval proof.
...
class PluginContext:
    ...
    def register_command(
        ...
        The default signature is ``fn(raw_args: str) -> str | None``.
        Handlers may opt in with ``command_context: PluginCommandContext | None``
        as a keyword parameter (or ``**kwargs``). CLI/TUI pass None; gateway
        metadata is not proof of a human or consent. Sync and async callables
        are supported. Handler errors are terminal, never agent prompts.
