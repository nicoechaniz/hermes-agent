...
class UniversalMetricParser:
    ...
    def _parse_stdout(self, stdout: str) -> ExperimentResults:
        """Parse stdout using regex: 'metric: value' and 'METRIC: key=value' formats."""
...
# Inline metric parser (ported from researchclaw.experiment.sandbox)
_FLOAT_RE = r"[+-]?\d+\.?\d*(?:[eE][+-]?\d+)?"
_METRIC_PATTERN = re.compile(
    rf"^(?:\S+=\S+\s+)?(\w[\w.]*)\s*:\s*({_FLOAT_RE})\s*$"
)
_HERMES_METRIC_PATTERN = re.compile(
    r"METRIC:\s*(\w[\w.]*)\s*=\s*(" + _FLOAT_RE + r")"
)


def _parse_metrics_from_stdout(stdout: str) -> dict[str, float]:
    """Extract metric: value pairs from stdout text."""
    metrics: dict[str, float] = {}
    for line in stdout.splitlines():
        line = line.strip()
        # Hermes format: "METRIC: key=value STATUS: ..."
        m = _HERMES_METRIC_PATTERN.search(line)
        if m:
            try:
                ...
                pass
            continue
        # Standard format: "metric_name: value"
