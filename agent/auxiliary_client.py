...
def resolve_provider_client(
    ...
    #
    # For ``provider == "auto"`` we intentionally do NOT pre-fill the model
    # with the user's main model. ``_resolve_auto`` first tries the main
    # provider with the main model (via ``main_runtime``), and only falls back
    # to other providers when that fails. Pre-filling ``model`` here would
    # force the fallback provider to receive the main model ID, which is
    # invalid for that provider (e.g. Gemini asked for ``kimi-k2.7-code``).
    # When the caller passes an explicit model we still respect it.
